...
init() {
      console.log("v. 0.0.4 ~ Giuseppe Di Menna 2026");
      ...
      fetch("/api/templates")
        ...
        .then(() => {
          // 2. CORREZIONE: Chiedi DIRETTAMENTE lo stato corrente del meeting.
          // Non chiamiamo più /api/meeting/start all'avvio, così non resettiamo i dati esistenti!
          fetch("api/meeting")
            .then((res) => res.json())
            .then((data) => {
              if (data) {
                ...
                // Imposta l'ID del template corrente per non far scattare il watch erroneamente
                if (data.id) {
      ...
      // Rimane l'ascolto in tempo reale dei cambi fatti dagli altri dispositivi
    ...
    setEndAuto() {
      ...
      const endTimestamp = startDate.getTime() + this.timers.map((t) => t.duration).reduce((a, b) => a + b, 0) * 1000;
      this.conferenceEnd = new Date(endTimestamp);
    },

    setStartAuto() {
      const endDate = this.conferenceEnd;
      const startTimpestamp = endDate.getTime() - this.timers.map((t) => t.duration).reduce((a, b) => a + b, 0) * 1000;
    ...
    postTimers() {
      ...
      fetch("/api/meeting", {
        ...
        body: JSON.stringify({
          ...
          endMeetingMode: this.endMeetingMode, // Inviato al server
